...
function AppointmentsContent() {
    ...
    async (date) => {
      ...
      try {
        ...
        const response = await fetch(
          `${API_URL}/api/appointments/date/${formattedDate}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
  ...
  const createAppointment = async () => {
    try {
      ...
      if (formData.manualEntry && !formData.patientName) {
        console.log(
          "DEBUG - Validation failed: Manual entry missing patient name"
        );
        toast({
          title: "Error",
      ...
      if (!formData.manualEntry && !formData.patient) {
        console.log(
          "DEBUG - Validation failed: Standard mode missing patient selection"
        );
        toast({
          title: "Error",
      ...
      if (formData.manualEntry) {
        payload = {
          patient: formData.patient, // Placeholder ObjectId
          patientName: formData.patientName,
      ...
      if (response.ok) {
        ...
        toast({
          ...
          description:
            data.message ||
            `Failed to create appointment. Status: ${response.status}`,
          variant: "destructive",
        });
      }
    } catch (error) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (
          ...
          formatDateForAPI(new Date(formData.date))
        ) {
          fetchAppointmentsByDate(selectedDate);
        }
      } else {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/appointments/search?query=${encodeURIComponent(
          searchQuery
        )}`,
        {
          headers: {
    ...
    const testAPIConnection = async () => {
      try {
        ...
        if (testResponse.ok) {
          ...
          console.log(
            "DEBUG - API test failed with status:",
            testResponse.status
          );
        }
      } catch (error) {
            ...
            {formData.manualEntry ? (
              ...
              <>
                <div className="grid gap-2">
                  ...
                  <Select
                    onValueChange={(value) =>
                      handleSelectChange("patient", value)
                    }
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value={APPOINTMENT_STATUS.SCHEDULED}>
                    {APPOINTMENT_STATUS.SCHEDULED}
                  </SelectItem>
                  <SelectItem value={APPOINTMENT_STATUS.IN_PROGRESS}>
